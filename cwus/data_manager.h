...
public:
    void loadDungeon() {
        ...
        for (const auto& room_data : j) {
            ...
            RoomData room(room_data.value("room_id", 0), room_data.value("floor", 0), room_data.value("room_type", 0),
...
public:// same situation as with RoomData
    ...
    const bool cast_on_enemy_;
    const int dmg_;
    const int periodic_dmg_;
    const bool add_evade_;
    const std::string description_;
    const std::string enemy_description_;
    WordData(int word_id, std::string word_name, bool cast_on_enemy, int dmg, int periodic_dmg,
        std::string description, std::string enemy_description, bool add_evade) :
        word_id_(word_id), word_name_(word_name), cast_on_enemy_(cast_on_enemy), dmg_(dmg), periodic_dmg_(periodic_dmg),
    ...
    void loadWords() {
        ...
        for (const auto& word_data : j) {
            WordData word(word_data.value("word_id", 0),
                ...
                word_data.value("cast_on_enemy", false),
...
public:
    ...
    const int chance_;
    EntityData(int max_hp, const std::string& name, const std::vector<int>& attack_pool, int chance) :
        hp_(max_hp), evade_(false), periodic_hp_change_(0), max_hp_(max_hp), name_(name),
...
public:
    void loadEntitys() {
        std::ifstream in_file("entities.json");
        if (!in_file.is_open()) { 
            std::cerr << "Не удалось открыть entities.json" << std::endl;
            return;
        }
        json j;
        in_file >> j;
        for (const auto& floor_data : j) {
            ...
            for (const auto& entity_data : floor_data["spawn_list"]) {
                EntityData entity = EntityData(entity_data.value("max_hp", 0),
                                               entity_data.value("name", ""), 
                                               entity_data.value("attack_pool",std::vector<int>{}), 
                                               entity_data.value("chance",1));
